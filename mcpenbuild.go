...
// generateTableOutput creates a table representation of catalog data
func generateTableOutput(data interface{}) (string, error) {
	var buffer bytes.Buffer
	table := tablewriter.NewWriter(&buffer)

	// Set table headers
	table.SetHeader([]string{"ID", "Name", "Type", "VCS", "Slug"})

	// Handle different data types
	switch v := data.(type) {
	case []*enbuild.Catalog:
		// Multiple catalogs
		for _, catalog := range v {
			table.Append([]string{
				fmt.Sprintf("%v", catalog.ID),
				catalog.Name,
				catalog.Type,
				catalog.VCS,
				catalog.Slug,
	...
	default:
		return "", fmt.Errorf("unsupported data type for table generation")
	}
